...
export function proxy(request: NextRequest) {
  ...
  if (guarded && !authenticated) return NextResponse.redirect(new URL('/login', request.url));
  if ((pathname === '/login' || pathname === '/verify-otp') && authenticated) return NextResponse.redirect(new URL('/home', request.url));
