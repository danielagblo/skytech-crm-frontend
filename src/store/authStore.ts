...
const isTokenValid = (token: string | null) => {
  ...
  if (token.startsWith('demo-')) return process.env.NEXT_PUBLIC_ENABLE_DEMO_AUTH === 'true';
  try {
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  clearAuth: () => {
    ...
    set({ user: null, accessToken: null, refreshToken: null, isAuthenticated: false, hydrated: true });
  },
  rehydrate: () => {
    ...
    if (!isTokenValid(accessToken)) {
      clearPersisted();
      set({ accessToken: null, refreshToken: null, isAuthenticated: false, hydrated: true });
      return;
    }
    set({ accessToken, refreshToken, isAuthenticated: Boolean(accessToken || refreshToken), hydrated: true });
  },
}));
